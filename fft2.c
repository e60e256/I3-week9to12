...
int mail(sample_t* buf, unsigned char* input, long n, long bpfmin, long bpfmax, double amp) {
  if (!pow2check(n)) {
...
//   print_complex(wp, Y, n);
    ...
    // y_l : 周期 T / l = 周波数 l / Tの波 = l / N
    //fprintf(stderr, "%ld, %ld\n", bpfmin, bpfmax);
    for (int i = 0; i < n/2; i++) {
      if ((44100.0 / n * i > bpfmax) | (44100.0 / n * i < bpfmin)) {
          Y[i] = 0;
      }
      Y[i] *= (complex double)amp;
    }


    for (int i = n/2; i < n; i++) {
      if ((44100.0 / n * (n - i) > bpfmax) | (44100.0 / n * (n - i) < bpfmin)) {
          Y[i] = 0;
      }
      Y[i] *= (complex double)amp;
    }
...
/*
unsigned short* data = (unsigned short*) malloc (DATA_SIZE * sizeof(unsigned short));
